...
/**
 * The {@link org.apache.hadoop.security.authentication.server.AuthenticationToken} contains information about an authenticated
 * HTTP client and doubles as the {@link java.security.Principal} to be returned by
 * authenticated {@link javax.servlet.http.HttpServletRequest}s
 * <p/>
 ...
 * done by the {@link org.apache.hadoop.security.authentication.server.AuthenticationFilter}).
 */
public class AuthenticationToken implements Principal {
   ...
   */
  public static final org.apache.hadoop.security.authentication.server.AuthenticationToken ANONYMOUS = new org.apache.hadoop.security.authentication.server.AuthenticationToken();
  ...
  public void setExpires(long expires) {
    if (this != org.apache.hadoop.security.authentication.server.AuthenticationToken.ANONYMOUS) {
  ...
  /**
   * Returns the principal name (this method name comes from the JDK {@link java.security.Principal} interface).
   *
   ...
   *
   * @throws org.apache.hadoop.security.authentication.client.AuthenticationException thrown if the string representation could not be parsed into
   * an authentication token.
   */
  public static org.apache.hadoop.security.authentication.server.AuthenticationToken parse(String tokenStr) throws AuthenticationException {
    Map<String, String> map = split(tokenStr);
    if (!map.keySet().equals(ATTRIBUTES)) {
      throw new AuthenticationException("Invalid token string, missing attributes");
    }
    long expires = Long.parseLong(map.get(EXPIRES));
    org.apache.hadoop.security.authentication.server.AuthenticationToken token = new org.apache.hadoop.security.authentication.server.AuthenticationToken(map.get(USER_NAME), map.get(PRINCIPAL), map.get(TYPE));
   ...
   *
   * @throws org.apache.hadoop.security.authentication.client.AuthenticationException thrown if the string representation of the token could not be broken into
   * attribute pairs.
   */
